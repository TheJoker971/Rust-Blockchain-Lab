...
fn test_keypair_generate() {
    ...
    assert!(keypair.private_key.starts_with("p"), "Private key should start with 'pk'");
}
...
fn test_keypair_from_private_key() {
    ...
    assert!(!keypair2.private_key.starts_with("pp"), "Private key should not start with 'pkpk'");
}